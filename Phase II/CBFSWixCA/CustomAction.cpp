      ...
      WcaLog(LOGMSG_STANDARD, "CBFSCab(g_hInst,IDR_CBFSINST_X32_DLL1)");
      {
         ...
         }
         {
            ...
            }
            {
               ...
               if ( FALSE == InstWrapper.Install(CBFSCab.m_szOutputFilename, W2CT(pszwProductName), TRUE, iModulesToInstall, &dwRebootNeeded) )
               {
      ...
      }
      {
         ...
         }
         {
            ...
            }
            {
               ...
               if ( FALSE == InstWrapper.Uninstall(CBFSCab.m_szOutputFilename, W2CT(pszwProductName), &dwRebootNeeded) )
               {
                  ExitWithLastError(hr, "Failed InstWrapper.Install");
               }
               else if ( dwRebootNeeded != 0 )
